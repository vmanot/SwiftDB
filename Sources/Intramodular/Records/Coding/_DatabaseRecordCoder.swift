    ...
    func decodeFieldPayload(forKey key: CodingKey) throws -> _RecordFieldPayload? {
        ...
        switch property {
            case is _Schema.Entity.Attribute:
                return try decodeUnsafeFieldValue(forKey: key).map(_RecordFieldPayload.init(from:))
            case is _Schema.Entity.Relationship:
                return try .relationship(
                    primaryKeysOrRecordIdentifiers: decodeRelatedPrimaryKeysOrRecordIDs(forKey: key)
                )
            default:
                throw _Error.unknownPropertyType(property.type, forKey: key)
        }
